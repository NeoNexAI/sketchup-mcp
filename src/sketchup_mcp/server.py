...
# NeoNexAI hardened fork of mhyrr/sketchup-mcp: eval_ruby (arbitrary code
# execution) removed; curated tool surface only. See README for details.
__version__ = "1.0.0"
logger.info(f"NeoNexAI SketchupMCP Server version {__version__} starting up")
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Curated tool surface (create/transform/delete components, materials, "
        "boolean ops, chamfer/fillet, woodworking joints, export). "
        "Arbitrary Ruby execution is intentionally NOT exposed."
    ),
...
# NOTE: The upstream `eval_ruby` tool (arbitrary Ruby code execution inside
# SketchUp — full disk/network access) has been intentionally REMOVED in this
# NeoNexAI fork. It was the reason the original MCP was flagged unsafe. All
# geometry operations below go through explicit, bounded tool handlers.
...
def boolean_operation(
    ...
    """Perform a boolean operation between two solid groups/components.

    operation: one of "union", "difference", "intersection".
    target_id / tool_id: entity IDs of the two solids.
    delete_originals: erase the source solids after the operation.
    """
    try:
        logger.info(f"boolean_operation called operation={operation}, target_id={target_id}, tool_id={tool_id}, delete_originals={delete_originals}")

        if operation not in ("union", "difference", "intersection"):
            return json.dumps({
                "success": False,
                "error": f"Invalid operation '{operation}'. Must be 'union', 'difference' or 'intersection'."
            })
        ...
        logger.info(f"boolean_operation result: {result}")
        return json.dumps(result)
    except Exception as e:
        logger.error(f"Error in boolean_operation: {str(e)}")
...
def chamfer_edges(
    ...
    """Chamfer (bevel) the edges of a solid group/component.

    entity_id: entity ID of the solid.
    distance: chamfer distance.
    edge_indices: optional list of edge indices to chamfer; omit for all edges.
    delete_original: replace the original solid with the chamfered result.
    """
    try:
        logger.info(f"chamfer_edges called entity_id={entity_id}, distance={distance}, edge_indices={edge_indices}, delete_original={delete_original}")
        ...
        logger.info(f"chamfer_edges result: {result}")
        return json.dumps(result)
    except Exception as e:
        logger.error(f"Error in chamfer_edges: {str(e)}")
        return f"Error chamfering edges: {str(e)}"
...
def fillet_edges(
    ...
    """Fillet (round) the edges of a solid group/component.

    entity_id: entity ID of the solid.
    radius: fillet radius.
    segments: number of segments around the rounded edge (smoothness).
    edge_indices: optional list of edge indices to fillet; omit for all edges.
    delete_original: replace the original solid with the filleted result.
    """
    try:
        logger.info(f"fillet_edges called entity_id={entity_id}, radius={radius}, segments={segments}, edge_indices={edge_indices}, delete_original={delete_original}")
        ...
        logger.info(f"fillet_edges result: {result}")
        return json.dumps(result)
    except Exception as e:
        logger.error(f"Error in fillet_edges: {str(e)}")
        return f"Error filleting edges: {str(e)}"
